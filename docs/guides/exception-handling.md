...
- [예외 계층 구조](#예외-계층-구조)
- [에러 코드 체계](#에러-코드-체계)
- [Domain Exception 사용 예제](#domain-exception-사용-예제)
- [Application Exception 사용 예제](#application-exception-사용-예제)
- [서비스 레이어 사용 예제](#서비스-레이어-사용-예제)
- [글로벌 예외 핸들러](#글로벌-예외-핸들러)
- [베스트 프랙티스](#베스트-프랙티스)
...
// Slot ID로 생성
throw new SlotNotFoundException(123L);

// Room ID, Date, Time으로 생성
throw new SlotNotFoundException(101L, "2025-01-15", "14:00");
```
...
// Slot ID와 상태
throw new SlotNotAvailableException(123L, "PENDING");
```
...
// 커스텀 메시지
throw new InvalidSlotStateTransitionException("이미 확정된 예약은 취소할 수 없습니다");
```
...
// Room ID로 생성
throw new PolicyNotFoundException(101L, true);
```
...
// 종료 날짜가 시작 날짜보다 이전
throw InvalidTimeRangeException.endDateBeforeStartDate("2025-01-20", "2025-01-15");

// 잘못된 기간
throw InvalidTimeRangeException.invalidDuration("-5 days");
```
...
// 필드명과 날짜
throw new PastDateNotAllowedException("startDate", "2024-01-01");
```
...
// 잘못된 슬롯 시간
throw InvalidScheduleException.invalidSlotTime("MONDAY", "25:00");

// 중복된 슬롯 시간
throw InvalidScheduleException.duplicateSlotTime("TUESDAY", "09:00");
```
...
// Room ID로 생성
throw SlotGenerationFailedException.forRoom(101L, cause);
```
